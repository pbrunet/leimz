	...
	public boolean checkMoving(Entity entity)
	{
		...
		if(!collision_possible.contains(true))
		{
			for(int i = 0; i < entities.size(); i++)
			{
				...
				{
					if(s.intersects(new Rectangle(entities.get(i).getPos_real_on_screen().x+entities.get(i).getPieds().getX(), entities.get(i).getPos_real_on_screen().y+entities.get(i).getPieds().getY(), entities.get(i).getPieds().getWidth(), entities.get(i).getPieds().getHeight())))
					{
						collision_possible.add(true);
					}
					else
					{
						collision_possible.add(false);
					}
				}
			}
		}
